...
CGFloat const DAPagesContainerTopBar_Height = 44.; // khanh add
...
- (void)setUp
{
    _topBarHeight = DAPagesContainerTopBar_Height;
...
- (void)setSelectedIndex:(NSUInteger)selectedIndex animated:(BOOL)animated
{
    ...
    TopBarItemView *previosSelectdItem = self.topBar.itemViews[self.selectedIndex];
    TopBarItemView *nextSelectdItem = self.topBar.itemViews[selectedIndex];
        ...
        [UIView animateWithDuration:(animated) ? 0.3 : 0. delay:0. options:UIViewAnimationOptionBeginFromCurrentState animations:^
         {
             ...
             previosSelectdItem.topViewLine.hidden = YES;
             nextSelectdItem.topViewLine.hidden = NO;
...
- (void)setPageItemsTitleColor:(UIColor *)pageItemsTitleColor
{
    if (![_pageItemsTitleColor isEqual:pageItemsTitleColor]) {
        ...
        TopBarItemView* itemView = self.topBar.itemViews[self.selectedIndex];
//        itemView.topViewLine.hidden = NO;
...
-(void) setImageViews:(NSArray *)imageViews {
    if(_imageViews != imageViews) {
        _imageViews = imageViews;
        self.topBar.imageViews = imageViews;
    }
}
...
- (void)itemAtIndex:(NSUInteger)index didSelectInPagesContainerTopBar:(DAPagesContainerTopBar *)bar
{
    ...
    if(self.delegate) {
        [self.delegate itemAtIndex:index didSelectInPagesContainerTopBar:bar];
    }
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    //self.selectedIndex = scrollView.contentOffset.x / 320;
    self.scrollView.userInteractionEnabled = YES;
}
                       ...
                       context:(void *)context
{
    ...
    if (oldX != self.scrollView.contentOffset.x && self.shouldObserveContentOffset) {
        ...
        if (targetIndex >= 0 && targetIndex < self.viewControllers.count) {
            ...
            TopBarItemView *previosSelectedItem = self.topBar.itemViews[self.selectedIndex];
            TopBarItemView *nextSelectedItem = self.topBar.itemViews[targetIndex];
...
//            previosSelectedItem.topViewLine.hidden = YES;
//            nextSelectedItem.topViewLine.hidden = NO;
